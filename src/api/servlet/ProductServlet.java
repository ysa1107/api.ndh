 ...
 * @author liemtpt
 */
public class ProductServlet extends HttpBasePage<ProductServlet> {

    private static final Logger logger = LogUtil.getLogger(ProductServlet.class);    
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if(FunctionUtils.checkAccessToken(request, response, mapParams)){
                mapParams.put("module", "products");
                int version = ConvertUtils.toInt(mapParams.get("version"), 1);    
                version = getVersion("products", version);
                
                String _packageControler = getPackageHandler(ProductServlet.class, version) ;

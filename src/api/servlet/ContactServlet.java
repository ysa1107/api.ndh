 ...
 * @author liemtpt
 */
public class ContactServlet extends HttpBasePage<ContactServlet> {

    private static final Logger logger = LogUtil.getLogger(ContactServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if(FunctionUtils.checkAccessToken(request, response, mapParams)){
                mapParams.put("module", "contacts");
                int version = ConvertUtils.toInt(mapParams.get("version"), 1);
                version = getVersion("contacts", version); 
                
                String _packageControler = getPackageHandler(ContactServlet.class, version);

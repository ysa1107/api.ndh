 ...
 * @author Y Sa
 */
public class RequestServlet extends HttpBasePage<RequestServlet> {

    private static final Logger logger = LogUtil.getLogger(RequestServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if(FunctionUtils.checkAccessToken(request, response, mapParams)){
                mapParams.put("module", "requests");
                int version = ConvertUtils.toInt(mapParams.get("version"), 1);
                version = getVersion("requests", version);
                
                String _packageController = getPackageHandler(RequestServlet.class, version);

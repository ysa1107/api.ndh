 ...
 * @author liemtpt
 */
public class CategoryServlet extends HttpBasePage<CategoryServlet> {

    private static final Logger logger = LogUtil.getLogger(CategoryServlet.class);    
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if(FunctionUtils.checkAccessToken(request, response, mapParams)){
                mapParams.put("module", "categories");
                int version = ConvertUtils.toInt(mapParams.get("version"), 1);    
                version = getVersion("categories", version);    
                String _packageControler = getPackageHandler(CategoryServlet.class, version) ;

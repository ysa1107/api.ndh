 ...
 * @author huanlh
 */
public class DealServlet extends HttpBasePage<DealServlet> {

    private static final Logger logger = LogUtil.getLogger(DealServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (FunctionUtils.checkAccessToken(request, response, mapParams)) {
                mapParams.put("module", "deals");
                int version = ConvertUtils.toInt(mapParams.get("version"), 1);
                version = getVersion("deals", version);

                String _packageControler = getPackageHandler(DealServlet.class, version);

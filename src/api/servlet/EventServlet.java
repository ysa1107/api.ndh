 ...
 * @author liemtpt
 */
public class EventServlet extends HttpBasePage<EventServlet> {

    private static final Logger logger = LogUtil.getLogger(EventServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (FunctionUtils.checkAccessToken(request, response, mapParams)) {
                mapParams.put("module", "events");
                int version = ConvertUtils.toInt(mapParams.get("version"), 1);
                version = getVersion("events", version);

                String _packageControler = getPackageHandler(EventServlet.class, version);

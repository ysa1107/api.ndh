 ...
 * @author liemtpt
 */
public class StoreServlet extends HttpBasePage<StoreServlet> {

    private static final Logger logger = LogUtil.getLogger(StoreServlet.class);    
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if(FunctionUtils.checkAccessToken(request, response, mapParams)){
                mapParams.put("module", "stores");
                int version = ConvertUtils.toInt(mapParams.get("version"), 1);    
                version = getVersion("stores", version);
                
                String _packageControler = getPackageHandler(StoreServlet.class, version) ;
